...
StateMachineMiiwaHelper::StateMachineMiiwaHelper(ArmManager* arm_manager, ArmManager* arm_manager_2):
sync_(false)
{
	arm_manager_ = arm_manager;
	arm_manager_2_ = arm_manager_2;

	ros::NodeHandle nh("~");
	joint_state_sub_ = nh_.subscribe("/joint_states", 1, &StateMachineMiiwaHelper::jointStatesCb, this);

}

void StateMachineMiiwaHelper::jointStatesCb(const sensor_msgs::JointState::ConstPtr &msg)
{
	joint_state_ = *msg;
...
StateMachineMiiwaHelper::~StateMachineMiiwaHelper()
{
	delete(arm_manager_);
}

void StateMachineMiiwaHelper::getDeliveryPlace(int frame, std::vector<double>& xyz, std::vector<double>& rpy)
{
...
int StateMachineMiiwaHelper::GraspObject(std::vector<double> pose)// This pose is given in base frame for now, afterwards we should modify to CAM frame
{	
	// *** Plan *** //
	...
	ROS_INFO("JOINTS: %f, %f ,  %f ,%f, %f, %f",joint_state_.position[0],joint_state_.position[1],joint_state_.position[2],joint_state_.position[3],joint_state_.position[4],
		joint_state_.position[5]);
...
void StateMachineMiiwaHelper::LeaveObject(int frame)// This pose is given in base frame for now, afterwards we should modify to CAM frame
{	
...
void StateMachineMiiwaHelper::moveSafePose()
{
